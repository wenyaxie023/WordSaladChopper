...
def setup_logger(log_path: str = None, level: int = logging.INFO):
...
def find_newline_token_ids(tokenizer, pattern="\n\n", include_special=False) -> list[int]:
    """Scan tokenizer vocab and return ids whose decoded text contains pattern (default: two newlines)."""
    ids: list[int] = []
    vocab_size = int(getattr(tokenizer, "vocab_size", 0) or 0)
    special_ids = set(tokenizer.all_special_ids) if not include_special else set()
    ...
    if hasattr(tokenizer, "added_tokens_decoder"):
        for tid in list(getattr(tokenizer, "added_tokens_decoder").keys()):
            if not include_special and tid in special_ids:
                continue
            try:
                text = tokenizer.decode([tid], skip_special_tokens=False)
            except Exception:
                continue
            if pattern in text and tid not in ids:
                ids.append(tid)

    return ids

def prepare_resume(run_dir: str,
                   samples: list[dict],
                   override: bool,
                   logger) -> tuple[list[dict], list[dict]]:
    """
    Load previous `results.json` if present and determine which samples remain.

    Returns
    -------
        all_results : list[dict]
        Previously saved records (may be empty).
        samples_to_run : list[dict]
        Samples still requiring generation.
    """
    # resume_path = run_dir / "results.json"
    resume_path = run_dir / "temp.json"
    all_results: list[dict] = []

    if resume_path.exists() and not override:
        try:
            with resume_path.open() as f:
                all_results = json.load(f)
            logger.info(f"Loaded {len(all_results)} prior results.")
        except Exception as e:
            logger.warning(f"Unable to read results.json ({e}); starting fresh.")
            all_results = []

    processed_ids = {r.get("id") for r in all_results}
    logger.info(f"processed_ids: {processed_ids}")
    samples_to_run = [s for s in samples if s.get("id") not in processed_ids]
    logger.info(f"samples_to_run: {len(samples_to_run)}")

    if not samples_to_run and not override:
        logger.info("All samples finished. Use --override to force rerun.")
    return all_results, samples_to_run
